...
function isLiked(idKuis) {
  const token = localStorage.getItem("token");
  return fetch(`http://localhost:3000/isLiked/${idKuis}`, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${token}`,
...
function searchKuis(kata) {
  fetch(`http://localhost:3000/kuis/search/${encodeURIComponent(kata)}`)
    .then((res) => res.json())
    .then((data) => {
      const token = localStorage.getItem("token");
